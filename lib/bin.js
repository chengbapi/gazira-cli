...
program
    ...
    .option('-c, --cache', 'initialize gazira but just build cache only')
    .description('initialize gazira for your project')
    .action(function(name, options) {
        name = name || 'static';
        if(options.cache) { // 只初始化cache，用于团队协作中其他人已经构建初始化好的情况
            console.log('start to build cache only');
            helper.createFolderIfNotExists(cachePath, function() {
                helper.git({
                    url: 'git@github.com:caolvchong/gazira.git',
                    dest: path.join(cachePath, 'gazira')
                }, function() {
                    console.log('build gazira cache successfully');
                });
            });
        } else {
            fs.exists(cachePath, function(exists) {
                if(exists) { // 存在缓存目录，说明已经被初始化过，之需要更新
                    console.log('gazira has been initialized before, and then will do update task');
                    helper.getGazira(name, false, options.dependencies);
                } else {
                    helper.createFolderIfNotExists(cachePath, function(flag) {
