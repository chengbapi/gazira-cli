...
var helper = {
     ...
     * @param {Function} callback 回调
     */
    deleteFolderIfExists: function (path, callback) {
        fs.exists(path, function (exists) {
            if (exists) {
                fs.remove(path, function (err) {
                    if (err) {
                        return console.error('deleteFolderIfExists', err);
                    }
                    callback && callback();
                });
            } else {
     ...
     * @param params
     *        {String} url git地址
     *        {String} dest clone到哪里
     * @param {Function} callback 回调
     */
    gitClone: function (params, callback) {
        var url = params.url;
        var dest = params.dest || '';

        helper.deleteFolderIfExists(dest, function () {
            ...
            git.stderr.on('data', function (data) {
                flag = false;
                console.error('gitClone', decoder.write(data));
            });
     ...
     * @param callback
     */
    getGazira: function (staticFolderName, installDependencies, callback) {
    ...
    removeCache: function() {
        fs.remove(cachePath, function(err) {
            if(err) {
                return console.error('remove', cachePath, 'error');
            }
                ...
                fs.exists(destPath, function (exists) {
                    if (exists) {
                        fs.copy(path.join(currentPath, '_static'), destPath, function (err) {
                            ...
                            helper.getGazira(name, options.dependencies, function () {
                                var ep = new EventProxy();
                                ep.all('removeCache', 'removeStatic', function () {
                                    console.log('clean temp files successfully');
                                });
                                fs.remove(cachePath, ep.done('removeCache'));
                                fs.remove(path.join(currentPath, '_static'), ep.done('removeStatic'));
                            });
                        })
                    } else {
                        fs.rename(path.join(currentPath, '_static'), destPath, function (err) {
                            ...
                            helper.getGazira(name, options.dependencies, helper.removeCache);
                        });
                    }
                });
            });
